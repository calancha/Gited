...
;; Last-Updated: Mon Mar 27 20:23:37 JST 2017
;;           By: calancha
;;     Update #: 551
;;
...
as well."
  ...
  (let ((br-after (gited-next-branch))
      ...
      (pcase gited-ref-kind
        ...
        ("local"
         (if (zerop (gited-git-command
                     `("branch" "--delete" ,(if force "--force" "") ,branch) buf))
             (progn
...
set RESET non-nil."
  ...
  (let ((bisectingp (gited-bisecting-p))
    ...
    (with-current-buffer obuf (read-only-mode 0))
    (cond (reset
           (if (not bisectingp)
    ...
    (gited-mark-if
     (and (not (eolp))
          ...
          (let* ((fn (gited-get-branchname))
                 ...
                 (time-min (time-subtract time-max (days-to-time days)))
                 (args (list "log"
                             (format "--after=%s"
